...
class DepthGuidedFeatureVolume(nn.Module):
    ...
    def forward(self, feats, depth_maps, source_poses, intrinsic_matrices):
        ...
        volume_features = torch.zeros(B, self.volume_reso, self.volume_reso, self.volume_reso, C, device=feats.device)
            ...
            for nv in range(NV):
                ...
                for y in range(H):
                    for x in range(W):
                        ...
                        distance_from_surface = torch.abs(point_3d[2] - depth_at_pixel)  # Z coordinate represents depth
                        ...
                        if distance_from_surface <= self.delta:
                            ...
                            if 0 <= voxel[0] < self.volume_reso and 0 <= voxel[1] < self.volume_reso and 0 <= voxel[2] < self.volume_reso:
                                volume_features[b, voxel[0], voxel[1], voxel[2]] += feats[b, nv, :, y, x]
