...
def log_images(rgb, rgb_gt, depth, depth_gt):
    """
    Log images to WandB with RGB and depth images concatenated with their ground truths.
    rgb, rgb_gt: (B, 3, H, W)
    depth, depth_gt: (B, H, W)
    """
    # Move tensors to CPU and concatenate predicted and ground truth depth images
    depth_combined = np.concatenate(
        (depth.cpu().numpy(), depth_gt.cpu().numpy()), axis=2
    )
    depth_combined_normalized = (
        (depth_combined / np.max(depth_combined, axis=(1, 2), keepdims=True)) * 255
    ).astype(np.uint8)

    # Move tensors to CPU and concatenate predicted and ground truth RGB images
    rgb_combined = np.concatenate((rgb.cpu().numpy(), rgb_gt.cpu().numpy()), axis=3)
    rgb_combined_transposed = np.transpose(rgb_combined, (0, 2, 3, 1))
    rgb_combined_normalized = (rgb_combined_transposed * 255).astype(np.uint8)

    # Create lists of images for logging
    depth_images = [
        wandb.Image(Image.fromarray(img)) for img in depth_combined_normalized
    ]
    rgb_images = [wandb.Image(Image.fromarray(img)) for img in rgb_combined_normalized]

    # Log images to WandB
    wandb.log({f"depth_images": depth_images, f"rgb_images": rgb_images})


def save_result(rgb, rgb_gt, depth, depth_gt, batch_idx):
    print("rgb", rgb.shape)
    ...
    for i in range(depth.shape[0]):
        ...
        Image.fromarray(depth_save).save(
            os.path.join("val_output/depth", "%d_%d_depth.png" % (batch_idx, i))
        )
    for i in range(depth.shape[0]):
        ...
        Image.fromarray(depth_save).save(
            os.path.join("val_output/depth_gt", "%d_%d_depth.png" % (batch_idx, i))
        )
    for i in range(rgb.shape[0]):
        save_image(rgb[i], "val_output/rgb/%d_%d_image.png" % (batch_idx, i))
    for i in range(rgb_gt.shape[0]):
        save_image(rgb_gt[i], "val_output/rgb_gt/%d_%d_image_gt.png" % (batch_idx, i))
...
class VolRecon(pl.LightningModule):
    def __init__(self, args):
        ...
        self.pos_encoding_2 = self.order_posenc(
            d_hid=16, n_samples=self.point_num + self.point_num_2
        )
    ...
    def sample2rgb(
        self, batch, points_x, z_val, ray_d, ray_idx, source_imgs_feat, feature_volume
    ):
        B, L, _, imgH, imgW = batch["source_imgs"].shape
        RN = ray_idx.shape[1]
        _, _, SN, _ = points_x.shape

        radiance, srdf, points_in_pixel = self.ray_transformer(
            points_x, batch, source_imgs_feat, feature_volume
        )

        ray_d = repeat(ray_d, "RN Dim3 -> RN SN Dim3", SN=SN)

        rgb, depth, opacity, weight, variance = self.renderer.render(
            rearrange(z_val, "B RN SN -> (B RN) SN"),
    ...
    def infer(
        ...
        if not extract_geometry:
            ...
            rgb_gt = torch.gather(
                ref_img, 2, repeat(ray_idx, "B RN -> B DimRGB RN", DimRGB=3)
            )
            rgb_gt = rearrange(rgb_gt, "B C RN -> B RN C")

            # gt depth for rays
            ref_depth = rearrange(
                batch["depths_h"][:, 0], "B H W -> B (H W)"
            )  # use only depth of reference view
            depth_gt = torch.gather(ref_depth, 1, ray_idx)

        ray_d = torch.gather(
            batch["ray_d"], 2, repeat(ray_idx, "B RN -> B DimX RN", DimX=3)
        )
        ...
        points_x_all = torch.gather(
            points_x_all, 2, repeat(sample_sort_idx, "B RN SN -> B RN SN 3")
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        # ---------------------- step 1: sample rays --------------------------------
        ray_idx = torch.argsort(
            torch.rand(B, imgH * imgW).type_as(batch["ray_o"]), dim=-1
        )[:, : self.train_ray_num]
        ...
        mask_depth = (
            (depth_gt != 0)
            & (depth_gt >= batch["near_fars"][:, 0, 0:1])
            & (depth_gt <= batch["near_fars"][:, 0, 1:2])
        )

        if torch.sum(mask_depth) > 0:
            # masked out where gt depth is invalid
            loss_depth_ray = torch.nn.functional.l1_loss(
                depth[mask_depth], depth_gt[mask_depth]
            )
            loss_depth_ray2 = torch.nn.functional.l1_loss(
                depth_2[mask_depth], depth_gt[mask_depth]
            )
        else:
            loss_depth_ray = loss_depth_ray2 = 0.0

        loss = self.args.weight_rgb * (
            loss_rgb + loss_rgb2
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # ---------------------- step 1: sample rays -----------------------------------------
        ray_idx_all = (
            repeat(torch.arange(imgH * imgW), "HW -> B HW", B=B)
            .type_as(batch["ray_o"])
            .long()
        )
        ...
        # log_images(
        #     batch_idx, rgb_imgs_2, rgb_gt_imgs, depths_2, batch["depths_h"][:, 0]
        # )
        # save_result(
        # rgb_imgs_2, rgb_gt_imgs, depths_2, batch["depths_h"][:, 0], batch_idx
        # )

        log_images(rgb_imgs_2, rgb_gt_imgs, depths_2, batch["depths_h"][:, 0])

        # metrics
        loss_rgb = torch.nn.functional.mse_loss(rgb_list, rgb_gt_list)
        loss_rgb_2 = torch.nn.functional.mse_loss(rgb_list_2, rgb_gt_list)

        psnr_coarse = piq.psnr(
            torch.clamp(rgb_imgs, max=1, min=0), torch.clamp(rgb_gt_imgs, max=1, min=0)
        ).item()
        psnr_fine = piq.psnr(
            torch.clamp(rgb_imgs_2, max=1, min=0),
            torch.clamp(rgb_gt_imgs, max=1, min=0),
        ).item()
        ...
        mask_depth = (
            (depth_gt != 0)
            & (depth_gt >= batch["near_fars"][:, 0:1, 0:1])
            & (depth_gt <= batch["near_fars"][:, 0:1, 1:2])
        )

        if torch.sum(mask_depth) > 0:
            # masked out where gt depth is invalid
            loss_depth_ray = torch.nn.functional.l1_loss(
                depths[mask_depth], depth_gt[mask_depth]
            )
            loss_depth_ray2 = torch.nn.functional.l1_loss(
                depths_2[mask_depth], depth_gt[mask_depth]
            )
        else:
            loss_depth_ray = loss_depth_ray2 = 0.0

        return {
            "val/loss_rgb_coarse": loss_rgb.item(),
    ...
    def extract_geometry(self, batch, batch_idx):
        ...
        for ray_idx in tqdm(torch.split(ray_idx_all, self.args.test_ray_num, dim=1)):
            srdf, points_x, depth, rgb = self.infer(
                batch=batch,
                ray_idx=ray_idx,
                source_imgs_feat=source_imgs_feat,
                feature_volume=feature_volume,
        ...
        depth_save = ((depths / np.max(depths)).astype(np.float32) * 255).astype(
            np.uint8
        )
        Image.fromarray(depth_save).save(
            os.path.join(self.args.out_dir, scan_name, "depth", "%s.png" % ref_view)
        )
        Image.fromarray(rgbs).save(
            os.path.join(self.args.out_dir, "rgb", scan_name, "%s.jpg" % ref_view)
        )

        extrinsic_np = batch["extrinsic_render_view"][0].cpu().numpy()

        np.save(
            os.path.join(self.args.out_dir, "depth", scan_name, "%s.npy" % ref_view),
            {
                "depth": depths,
                "extrinsic": extrinsic_np,
                "intrinsic": batch["intrinsic_render_view"][0].cpu().numpy(),
            },
        )
