...
class MVSDataset(Dataset):
    def __init__(
        ...
        if img_wh is not None:
            assert (
                img_wh[0] % 32 == 0 and img_wh[1] % 32 == 0
        ...
        (
            self.metas,
            self.ref_src_pairs,
        ) = self.build_metas()  # load ref-srcs view pairs info of the scene
            ...
            for scan in self.scans:
                with open(self.pair_filepath) as f:
                    ...
                    for _ in range(num_viewpoint):
                        ...
                        # ! only for validation
                        if (
                            len(self.test_ref_views) > 0
                            and ref_view not in self.test_ref_views
                        ):
                            continue
    ...
    def load_cam_info(self):
        for vid in range(self.num_all_imgs):
            ...
            intrinsic[
                :2
            ] *= 4  # * the provided intrinsics is 4x downsampled, now keep the same scale with image
    ...
    def save_depth(self, sample):
        ...
        for i in range(sample["depths_h"].shape[0]):
            ...
            Image.fromarray(depth_save).save(
                os.path.join("temp_output/depth", "%d_depth.png" % i)
            )
        for i in range(sample["depths_prior_h"].shape[0]):
            depth_prior = sample["depths_prior_h"][i, :, :].cpu().numpy()
            depth_save = (
                (depth_prior / np.max(depth_prior)).astype(np.float32) * 255
            ).astype(np.uint8)
            Image.fromarray(depth_save).save(
                os.path.join("temp_output/depth", "%d_prior.png" % i)
            )
    ...
    def __getitem__(self, idx):
        ...
        new_depths_h = []
        new_depths_prior_h = []
        for intrinsic, extrinsic, depth, depth_prior in zip(
            intrinsics, w2cs, depths_h, depths_h_prior
        ):
        ...
        sample["source_c2ws"] = sample["c2ws"][1:]
        sample["near_fars"] = torch.from_numpy(near_fars.astype(np.float32))[
            start_idx:
        ]  # (V, 2)
        sample["intrinsics"] = torch.from_numpy(intrinsics.astype(np.float32))[
            start_idx:, :3, :3
        ]  # (V, 3, 3)
        sample["source_intrinsics"] = sample["intrinsics"][1:]

        sample["meta"] = (
            str(scan) + "_light" + str(light_idx) + "_refview" + str(ref_view)
        )
        ...
        sample["source_depths_h"] = sample["depths_h"][1:]
