...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp_name", dest="exp_name", type=str, help="experiment name")
    parser.add_argument(
        "--root_dir", dest="root_dir", type=str, help="directory of training dataset"
    )
    parser.add_argument(
        "--load_ckpt",
        dest="load_ckpt",
    ...
    parser.add_argument(
        "--batch_size", dest="batch_size", type=int, default=2, help="batch size"
    )
    parser.add_argument(
        "--max_epochs",
        dest="max_epochs",
    ...
    parser.add_argument(
        "--val_only", dest="val_only", action="store_true", help="only validate"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=96,
        help="3D feature volume resolution",
    )  # set as 0 to disable
    ...
    # -------------------------------- args for testing --------------------------------
    parser.add_argument(
        "--test_dir", dest="test_dir", type=str, help="directory of test dataset"
    )
    parser.add_argument(
        "--out_dir", dest="out_dir", type=str, help="directory of to save test result"
    )
    parser.add_argument(
        "--extract_geometry",
        dest="extract_geometry",
        action="store_true",
    ...
    parser.add_argument(
        "--test_sample_coarse", dest="test_sample_coarse", type=int, default=64
    )
    parser.add_argument(
        "--test_sample_fine", dest="test_sample_fine", type=int, default=64
    )
    parser.add_argument(
        "--test_coarse_only",
        dest="test_coarse_only",
        action="store_true",
        ...
        if not args.test_general:
            for scan in [
                ...
                dataloader_tmp = DataLoader(
                    dataset_tmp, batch_size=1, num_workers=1, shuffle=False
                )
                dataloader_test.append(dataloader_tmp)
        else:
            for scan in ["general"]:
                dataset_tmp = GeneralFit(
                    root_dir=args.test_dir, scan_id=scan, n_views=args.test_n_view
                )
                dataloader_tmp = DataLoader(
                    dataset_tmp, batch_size=1, num_workers=1, shuffle=False
                )
    ...
    # -------------------------------- lightning module -------------------------------
    if args.load_ckpt:
        volrecon = VolRecon.load_from_checkpoint(
            checkpoint_path=args.load_ckpt, args=args
        )
